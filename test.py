...
# 加载指定模型
model.load_weights('./train_model/model_03-0.76.hdf5')
# 直接使用predict方法进行预测
y_pred = model.predict(np.array(test_images)[:10])
y_pred
pred = np.argmax(y_pred,axis=-1)
y_test[:10]
print('预测值：' + ' '.join('%5s' % pred[j] for j in range(len(pred))))
print('实际值：' + ' '.join('%5s' % y_test[j] for j in range(len(pred))))
